...
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	if (formBuy) {
		...
		if (formBuy.elements['number']) {
			number = parseInt(formBuy.elements['number'].value);
		}
		quick = 1;
	} else {
    ...
    Ajax.call('flow?step=add_to_cart', 'goods=' + JSON.stringify(goods), addToCartResponse, 'POST', 'JSON');
	}
...
function addToCartNums(goodsId, parentId) {
	...
	for (i = 0; i < goodsIds.length; i++) {
		...
		if (i == buynum) {
			Ajax.call('flow?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
		} else {
			Ajax.call('flow?step=add_to_cart', 'goods=' + $.toJSON(goods), '', 'POST', 'JSON');
		}
...
//			document.getElementById('tell_me_form').style.display = 'block';
		...
		var cart_url = domain_url+'flow?step=cart';
...
function MoveBox(gid) {
	...
	if (obj1.length > 0) {
		...
		// 购物车页面加入操作，刷新页面
		location.href = domain_url+'flow?step=cart';
	}
}
 ...
 * 添加商品到收藏夹
 */
function collect(goodsId, act) {
	//Ajax.call('user?act=collect', 'id=' + goodsId, collectResponse, 'GET', 'JSON');
	Ajax.call('user?act=collect', 'id=' + goodsId, collectResponse2, 'GET', 'JSON');

  function collectResponse2(result) {
    if (result.error > 0) {
      alert(result.message);
    } else {
      flyCollect(result.goods_id, 'collectGoods');// 飞入收藏
      if (act === 're_collect') {
        document.getElementById("collect_" + result.goods_id).className = "collet-btn collet-btn-t";
      } else if (act === 'g_collect') {
        var obj = document.getElementById("g_collect");
        obj.className = "btn-coll btn-coll-t";
        obj.innerHTML = "已收藏";
      }
    }
  }
 ...
 * 处理收藏商品的反馈信息
 */
function collectResponse(result, re_collect) {
	if (result.error > 0) {
		alert(result.message);
	} else {
		flyCollect(result.goods_id, 'collectGoods');// 飞入收藏
